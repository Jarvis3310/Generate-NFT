...
def parse_config():
    ...
    for layer in CONFIG:
        ...
        if layer['rarity_weights'] == 'None':
            ...
            assert len(traits) == len(layer['rarity_weights']), f"確保您擁有當前數量的稀有權重(圖片數:{len(traits)},權重數:{len(layer['rarity_weights'])},應該將config.yaml其中設置相同)"
            if len(traits) != len(layer['rarity_weights']):
                Logger('./log/error.log', level='error').logger.error(f"確保您擁有當前數量的稀有權重(圖片數:{len(traits)},權重數:{len(layer['rarity_weights'])},應該將config.yaml其中設置相同)")
                sys.exit()
            rarities = layer['rarity_weights']
        else:
            Logger('./log/error.log', level='error').logger.error("無效稀有權重")
            #raise ValueError("無效稀有權重")
            sys.exit()
...
def generate_images(edition, count, drop_dup=False):
    ...
    log.logger.info("生成第 %i 張圖片, %i張不同" % (count, rarity_table.shape[0]))
    if drop_dup:
        img_tb_removed = sorted(list(set(range(count)) - set(rarity_table.index)))
        log.logger.info("移除 %i 張圖片..." % (len(img_tb_removed)))
...
def main():
    global log
    folder_path = "./log"
    if not os.path.exists(folder_path):  # 判斷是否存在文件夾中，如果不存在則建立文件夾
        os.makedirs(folder_path)
    log = Logger('./log/all.log', level='info')
    log.logger.info('檢查配置...')
    ...
    log.logger.info('檢查素材...')
    parse_config()
    tot_comb = get_total_combinations()
    log.logger.info("您可以創建總共%i個不同的NFT" % (tot_comb))
    log.logger.info("您希望創建多少個NFT？輸入一個大於0的數字:")
    while True:
        num_avatars = int(input())
        if num_avatars > 0:
            break
    log.logger.info(f"建立{num_avatars}個NFT")
    log.logger.info("您想把該NFT項目命名為:")
    edition_name = input()
    log.logger.info(f"儲存NFT文件夾名:{edition_name}")
    log.logger.info("開始生成...")
    rt = generate_images(edition_name, num_avatars,config["drop_dup"])
    log.logger.info("保存metadata...")
